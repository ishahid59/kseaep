...
<!-- <h2 style="margin-top:5px;margin-bottom:7px;color:#000">Detail: <span class="small">{{project.ProjectNo}}</span></h2> -->
    ...
    <div class="row" style="margin:0px;padding:0px;margin-bottom:4px;">
        <div class="col-sm-3" style="padding-left:0px;">
              <h2 style="margin-top: 10px; margin-bottom: 1px; font-size:25px;font-family: 'Helvetica', Arial, sans-serif;">Project Detail : <span class="small">{{project.ProjectNo}}</span></h2>
        </div>
      ...
      >
        <div class="col-sm-5">
          <dl class="dl-horizontal" >
            ...
            <dt>Project Role</dt>
            <dd id="ddProjectRole">{{project.ProjectRole}}</dd>
            <dt>Award Year</dt>
            <dd id="ddAwardYear">{{project.AwardYear}}</dd>
            <dt>Owner Category</dt>
            <dd id="ddOwnerCategory">{{project.OwnerCategory}}</dd>
            <dt>Owner</dt>
            <dd id="ddOwner">{{project.Owner}}</dd>
            <dt>Client</dt>
            <dd id="ddClient">{{project.Client}}</dd>
            <dt>Company Name</dt>
            <dd id="ddComID">{{project.ComID}}</dd>
        ...
        <div class="col-sm-4"  >
          <dl class="dl-horizontal" style=" margin-top:0; margin-bottom:0px;">
            ...
            <dd id="ddPrimaryProjectType">{{project.PrimaryProjectType}}</dd>
            <dt>Secondary Pro. Type</dt>
            <dd id="ddSecondaryProjectType">{{project.SecondaryProjectType}}</dd>            
            <dt>Project Manager</dt>
            <dd id="ddProjectManager">{{project.ProjectManager}}</dd>
            <dt>Project Agreement No.</dt>
            <dd id="ddProjectAgreementNo">{{project.ProjectAgreementNo}}</dd>
            <dt>Proposal No.</dt>
            <dd id="ddProposalID">{{project.ProposalID}}</dd>
            <dt>Project Status</dt>
            <dd id="ddProjectStatus">{{project.ProjectStatus}}</dd>     
        ...
        <div class="col-sm-3 pull-right" style="margin-left:0px;padding-right:0px;">
          ...
          <button id="refreshproject" @click="refreshprodetail()" hidden>Refresh Project</button> -->
            ...
            <div style="width:100%;"> &nbsp; 
                <a @click="backtoproject()" class="pull-right" style="font-size:12px;color:#006fae;cursor:pointer">Back to Project List </a>
            </div>  
           ...
           <div style="width:100%;padding-top:165px;"> &nbsp;     
                ...
                <a @click="showproeditmodal()" type="button" class="btn btn-default btn-sm pull-right" style="color:#006fae;"><i class="fa fa-edit" style="font-size:14px;color:green;"></i> Edit Project</a>   -->
                 <a @click="proconfirmdelete()" type="button" class="btn btn-danger btn-sm pull-right" style="margin-left:6px;"> Delete Project </a>  
                <a @click="showproeditmodal()" type="button" class="btn btn-primary btn-sm pull-right" > Edit Project</a>                
          ...
          <div class="row">
             <div class="col-sm-8" style="padding-right:0px;">
              ...
              <div class="col-sm-4" style="padding-top:7px;padding-left:5px;">
                  <!-- src='images/prophoto/1990-0238/big1.jpg' -->
                  <!-- style="width:100%;border:1px solid #b6b6b6;background:url('images/ajax-loader.gif');" -->
                 <img
                id="test2"
                  :src="imagepath"
                  style="width:100%;border:1px solid #b6b6b6;background:url('images/ajax-loader.gif');background-repeat:no-repeat;"
                  alt
                />
              </div>
...
export default {
  ...
  mounted() {
            ...
            // axios
            // .get("apiprophotogetimagedata/" + this.$route.params.projectid + "").then(response => {
...
$('#test2').show();
   ...
   // get empid from global variable when page is refreshed since previous id is lost and becomes null with refreshed
    ...
    // to show the photo of the first row of the table when table loads
            axios
    ...
    axios.get("apilstProjectID").then((response) => {
